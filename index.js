...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        <p><strong>Name:</strong> ${games[i].name}</p>
...
const totalContributions = GAMES_JSON.reduce((acc, total) => {
    return acc + total.backers;
}, 0);
...
const totalAmountRaised = GAMES_JSON.reduce((acc, total) => {
    return acc + total.pledged;
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const unfundedGamesStr = `A total of $${totalAmountRaised.toLocaleString(
    "en-US"
)} has been raised for ${GAMES_JSON.length} games. Currently, ${
    unfundedGames.length < 2
        ? unfundedGames.length + " game"
        : unfundedGames.length + " games"
} remains unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
const newElement = document.createElement("p");
newElement.innerHTML = unfundedGamesStr;
